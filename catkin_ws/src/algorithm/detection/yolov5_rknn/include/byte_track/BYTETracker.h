...
{
public:
	BYTETracker(int frame_rate=30, float _track_thresh=0.5, float _high_thresh=0.6, float _match_thresh=0.8, int track_buffer = 30);
	~BYTETracker();

	 std::vector<STrack> update(const detect_result_group_t& objects, int target_class_id);
